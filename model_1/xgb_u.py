...
def model(df_train, df_test, drop_column):
    ...
    df_pred.drop_duplicates(['user_id'], keep='first', inplace=True)
    ...
    if os.path.exists(dump_path):
        ...
        df_real = feat_buy(end_date + timedelta(days=1), end_date + timedelta(days=7))[
            ['user_id']]
        df_real.drop_duplicates(['user_id'], keep='first', inplace=True)
        df_real.to_csv(dump_path, index=False)

    df_pred = df_pred[['user_id', 'pred']]
    print('前%s行[test] label=1：' % (str(df_real.shape[0])))
    report_u(df_real, df_pred.iloc[:df_real.shape[0]])
...
def submit(df_sub, drop_column):
    ...
    if os.path.exists(dump_path):
        ...
        df_pred = df_pred[['user_id']]
        df_pred=df_pred.iloc[:160000]
        df_pred.to_csv(submit_path + '/u.csv', index=False)
        print('<< 完成提交模型')
    else:
        print('<< 没有训练模型')
