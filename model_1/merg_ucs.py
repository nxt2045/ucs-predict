...
def gen_feat(end_date, label_gap, mark):
    ...
    dump_path = cache_path + '/feat_ucs_%s_%s_%s.csv' % (str(label_gap), mark, end_date.strftime('%y%m%d'))
...
def gen_label(end_date, label_gap, mark):
    ...
    print('> 获取标签')
    if mark == 'submit':
        user_cate = pd.read_csv(submit_path + '/uc.csv', na_filter=False)
        product = pd.read_csv(product_path, na_filter=False, usecols=['cate', 'shop_id'])
        product = product.drop_duplicates(['cate', 'shop_id'])
        pkey = pd.merge(user_cate, product, on='cate', how='left')
        ...
        # 可能购买
        user_cate = feat_buy_plus(end_date + timedelta(days=1), end_date + timedelta(days=7))[['user_id', 'cate']]
        product = pd.read_csv(product_path, na_filter=False, usecols=['cate', 'shop_id'])
