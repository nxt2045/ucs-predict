...
def feat_user_shop_user_action_ratio(start_date, end_date):
    ...
    if os.path.exists(dump_path):
        ...
        feat['user_shop_user_view_ratio'] = feat['user_shop_view_amt'] / (feat['user_view_amt']) * 100

        feat.ix[feat['user_buy_amt'] == 0, 'user_buy_amt'] = -1
        feat['user_shop_user_buy_ratio'] = feat['user_shop_buy_amt'] / (feat['user_buy_amt']) * 100

        feat.ix[feat['user_follow_amt'] == 0, 'user_follow_amt'] = -1
        feat['user_shop_user_follow_ratio'] = feat['user_shop_follow_amt'] / (feat['user_follow_amt']) * 100

        feat.ix[feat['user_remark_amt'] == 0, 'user_remark_amt'] = -1
        feat['user_shop_user_remark_ratio'] = feat['user_shop_remark_amt'] / (feat['user_remark_amt']) * 100

        feat.ix[feat['user_cart_amt'] == 0, 'user_cart_amt'] = -1
        feat['user_shop_user_cart_ratio'] = feat['user_shop_cart_amt'] / (feat['user_cart_amt']) * 100

        feat = feat[
            ['user_id', 'shop_id', 'user_shop_user_view_ratio', 'user_shop_user_buy_ratio',
             'user_shop_user_follow_ratio', 'user_shop_user_remark_ratio', 'user_shop_user_cart_ratio']]
        feat = feat.astype(int)
        feat.to_csv(dump_path, index=False)
