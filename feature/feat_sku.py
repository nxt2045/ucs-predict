...
def feat_sku_if_action(start_date, end_date):
    ...
    if os.path.exists(dump_path):
        ...
        feat.to_csv(dump_path, index=False)
        plt.figure()  # 建立图像
        feat.boxplot()  # 画箱线图，直接使用DataFrame的方法
    return feat
...
def feat_sku_comment_amt(start_date, end_date):
    ...
    if os.path.exists(dump_path):
        ...
        comment = comment[
            (start_date <= comment['dt']) & (comment['dt'] < end_date + timedelta(days=1))]
...
def feat_user_sku_wait(start_date, end_date):
    ...
    if os.path.exists(dump_path):
        ...
        feat = feat.drop_duplicates(subset=['user_id', 'sku_id'], keep=False)
        feat = pd.concat([feat.reset_index(drop=True), pd.DataFrame({'user_sku_wait': [1] * feat.shape[0]})], axis=1)
