...
def verify(df_verify, drop_column):
    """ 验证
    xgboost模型验证
    """
    dump_path = './out/bst.model'
    if os.path.exists(dump_path):
        ...
        print(datetime.now())
        print('>> 开始划分X,y')
        X_verify = df_verify.drop(drop_column, axis=1).values
        print('<< 完成划分数据')

        # 验证模型
        print('>> 开始加载模型')
        dverify = xgb.DMatrix(X_verify)
        bst = xgb.Booster(model_file=dump_path)
        y_probab = bst.predict(dverify)
        print('> 概率转换0,1')
        df_pred = pd.concat([df_verify, pd.DataFrame({'probab': y_probab, 'pred': [0] * len(y_probab)})], axis=1)
        df_pred = df_pred.sort_values(by='probab', ascending=False)
        df_pred.ix[:int(np.sum(df_verify['label'].values)), 'pred'] = 1
        print('> 保存结果')
        df_pred.to_csv('./out/verify_pred.csv', index=False)
        f11_score(df_pred['label'], df_pred['pred'])
        print('<< 完成验证模型')
    else:
        print('<< 没有模型')
