...
print('\t读取完成！用时', time.clock() - time_0, 's')
# 总hist
'''
print("> histing")
plt.rcParams['figure.figsize'] = (15, 10)
product.hist(bins=20)
plt.savefig(plot_path + '/product_hist.png', dpi=300)
user.hist(bins=20)
plt.savefig(plot_path + '/user_hist.png', dpi=300)
action.hist(bins=20)
plt.savefig(plot_path + '/action_hist.png', dpi=300)
comment.hist(bins=20)
plt.savefig(plot_path + '/comment_hist.png', dpi=300)
shop.hist(bins=20)
plt.savefig(plot_path + '/shop_hist.png', dpi=300)
'''
# buy_product
# buy_time buy_cate
buy = action[action['type'] == 2]
buy_product = pd.merge(buy, product, on=['sku_id'])
