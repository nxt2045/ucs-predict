...
def get_action_u0524_feat1(start_date, end_date):
    ...
    if os.path.exists(dump_path):
        ...
        actions['time_diff_early'] = actions['time_diff_early'].dt.days * 24 + actions[
            'time_diff_early'].dt.seconds // 3600
        ...
        sub_actions['sub_time_diff_early'] = sub_actions['sub_time_diff_early'].dt.days * 24 + sub_actions[
            'sub_time_diff_early'].dt.seconds // 3600
...
def get_action_u0524_feat2(start_date, end_date):
    ...
    if os.path.exists(dump_path):
        ...
        actions['time_diff_recent'] = actions['time_diff_recent'].dt.days * 24 + actions[
        ...
        sub_actions['sub_time_diff_recent'] = sub_actions['sub_time_diff_recent'].dt.days * 24 + sub_actions[
            'sub_time_diff_recent'].dt.seconds // 3600
...
def user_features(user, ful_action, sub_action, end_date):
    ...
    if os.path.exists(dump_path):
        ...
        col = ['user_id', 'us_browse_all_avg', 'us_add_all_avg', 'us_del_all_avg', 'us_buy_all_avg',
               'us_follow_all_avg', 'us_click_all_avg', 'us_action_all_avg']
        us_avg.columns = col
        us_max = us.groupby('user_id').max().reset_index()
        col = ['user_id', 'us_browse_all_max', 'us_add_all_max', 'us_del_all_max', 'us_buy_all_max',
               'us_follow_all_max', 'us_click_all_max', 'us_action_all_max']
        us_max.columns = col
        ...
        col = ['user_id', 'sku_browse_all_avg', 'sku_add_all_max', 'sku_del_all_max', 'sku_buy_all_max',
               'sku_follow_all_max', 'sku_click_all_max', 'sku_action_all_max']
        sku_avg.columns = col

        sku_min = sku.groupby('user_id').min().reset_index()
        col = ['user_id', 'sku_browse_all_min', 'sku_add_all_min', 'sku_del_all_min', 'sku_buy_all_min',
               'sku_follow_all_min', 'sku_click_all_min', 'sku_action_all_min']
        sku_min.columns = col
        ...
        col = ['user_id', 'browse_ful_all', 'add_ful_all', 'del_ful_all', 'buy_ful_all', 'follow_ful_all',
               'click_ful_all', 'action_ful_all']
        ...
        col = ['user_id', 'browse_ful_30', 'add_ful_30', 'del_ful_30', 'buy_ful_30', 'follow_ful_30', 'click_ful_30',
               'action_ful_30']
        ...
        us = us.drop('sku_id', axis=1)
        us_avg = us.groupby('user_id').mean().reset_index()
        col = ['user_id', 'us_browse_5_avg', 'us_add_5_avg', 'us_del_5_avg', 'us_buy_5_avg', 'us_follow_5_avg',
               'us_click_5_avg', 'us_action_5_avg']
        us_avg.columns = col
        us_max = us.groupby('user_id').max().reset_index()
        col = ['user_id', 'us_browse_5_max', 'us_add_5_max', 'us_del_5_max', 'us_buy_5_max', 'us_follow_5_max',
               'us_click_5_max', 'us_action_5_max']
        us_max.columns = col
        ...
        col = ['user_id', 'browse_ful_5', 'add_ful_5', 'del_ful_5', 'buy_ful_5', 'follow_ful_5', 'click_ful_5',
               'action_ful_5']
...
def make_test_set(train_start_date, train_end_date, user, ful_action, sub_action):
    ...
    if os.path.exists(dump_path):
        ...
        print(actions.shape)
        actions = pd.merge(actions, user_features(user, ful_action, sub_action, train_end_date), how='left',
                           on='user_id')
        print(actions.shape)
        actions = pd.merge(actions, get_action_user_feat0515_2_1(train_start_date, train_end_date), how='left',
                           on='user_id')
        print(actions.shape)
        actions = pd.merge(actions, get_action_user_feat0515_2_2(train_start_date, train_end_date), how='left',
                           on='user_id')
        print(actions.shape)

        # 模型1   和 模型二
        actions = pd.merge(actions, get_action_u0509_feat_29(train_start_date, train_end_date), how='left',
                           on='user_id')
        print(actions.shape)
        # 模型 二
        actions = pd.merge(actions, get_action_u0521_feat_32(train_start_date, train_end_date), how='left',
                           on='user_id')
        ...
        for i in (1, 2, 3, 7, 14, 28):
            actions = pd.merge(actions, get_action_user_feat_six_xingwei(train_start_date, train_end_date, i),
                               how='left', on='user_id')
            actions = pd.merge(actions, deal_user_six_deal(train_start_date, train_end_date, i), how='left',
                               on='user_id')
            actions = pd.merge(actions, get_action_user_feat11(train_start_date, train_end_date, i), how='left',
                               on='user_id')
            actions = pd.merge(actions, get_action_user_feat13(train_start_date, train_end_date, i), how='left',
                               on='user_id')
            actions = pd.merge(actions, get_action_user_feat0509_1_30(train_start_date, train_end_date, i), how='left',
                               on='user_id')
            actions = pd.merge(actions, get_action_user_feat0515_2_3(train_start_date, train_end_date, i), how='left',
                               on='user_id')
            actions = pd.merge(actions, get_action_feat(train_start_date, train_end_date, i), how='left', on='user_id')
            actions = pd.merge(actions, get_action_user_feat0515_2_4(train_start_date, train_end_date, i), how='left',
                               on='user_id')
            actions = pd.merge(actions, get_action_u0515_feat5(train_start_date, train_end_date, i), how='left',
                               on='user_id')
            # 模型1   和 模型二
            actions = pd.merge(actions, get_action_u0509_feat_28(train_start_date, train_end_date, i), how='left',
                               on='user_id')
            if (i <= 10):
                actions = pd.merge(actions, get_action_user_feat0509_1_31(train_start_date, train_end_date, i),
                                   how='left', on='user_id')
            # 模型 二
            actions = pd.merge(actions, get_action_u0521_feat_31(train_start_date, train_end_date, i), how='left',
                               on='user_id')
            actions = pd.merge(actions, get_action_user_feat7_0522_huachuang(train_start_date, train_end_date, i),
                               how='left', on='user_id')
        print(actions.shape)
        print(actions.shape)
...
def make_train_set(train_start_date, train_end_date, test_start_date, test_end_date, user, ful_action, sub_action):
    dump_path = './cache/bu10525model_2_u_train_set_%s_%s_%s_%s.csv' % (
