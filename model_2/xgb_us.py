...
from model_2.merg_us import gen_feat
...
def model(df_train, df_test, drop_column):
    ...
    df_pred = df_pred.drop_duplicates(['user_id', 'sku_id'], keep='first')
    df_pred = df_pred.reset_index(drop=True)
    product = pd.read_csv(product_path, na_filter=False)[['sku_id', 'cate', 'shop_id']]
    ...
    report_ucs(df_real, df_pred.iloc[:df_real.shape[0]])

    for amt in range(80000, 280000, 10000):
        print('前%s行 label=1：' % (str(amt)))
        report_ucs(df_real, df_pred.iloc[:amt])
    print('<< 完成测试模型')
...
def submit(df_sub, drop_column):
    ...
    if os.path.exists(dump_path):
        ...
        df_pred.to_csv(submit_path + '/A2.csv', index=False)
